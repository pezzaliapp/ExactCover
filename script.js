document.addEventListener('DOMContentLoaded', () => {
  ...
  const requiredEls = [
    boardEl, presetSel, wInput, hInput, newBtn, holesBtn, resetHolesBtn,
    checkBtn, suggestBtn, solveOneBtn, findAllBtn, prevBtn, nextBtn,
    applyBtn, shuffleBtn, pieceSelect, rotBtn, flipBtn, removeBtn,
    clearPlacedBtn, palette, statSize, statusEl
  ];
  if (requiredEls.some(el => !el)) {
    console.error("Uno o più elementi DOM non trovati. Assicurati che l'HTML sia corretto.");
    return;
  }
  ...
  function normalize(c) {
    let minr = Math.min(...c.map(p => p[0])), minc = Math.min(...c.map(p => p[1]));
    const m = c.map(([r, cc]) => [r - minr, cc - minc]);
    ...
    for (let r = 0; r < H; r++) {
      for (let c = 0; c < W; c++) {
        ...
        cell.addEventListener('pointerdown', (e) => {
          ...
          DRAG = {
            L, shape: (obj.shape || normalize(PENTOMINOES[L])).map(x => x.slice()),
  ...
  window.addEventListener('keydown', e => {
    if (!DRAG) return;
    if (e.key === 'r' || e.key === 'R') {
      DRAG.shape = rotate(DRAG.shape);
      updatePreviewFromEvent({ clientX: window.lastX || 0, clientY: window.lastY || 0 });
      e.preventDefault();
    }
    if (e.key === 'f' || e.key === 'F') {
      DRAG.shape = reflect(DRAG.shape);
      updatePreviewFromEvent({ clientX: window.lastX || 0, clientY: window.lastY || 0 });
      e.preventDefault();
    }
  });
  ...
  function generatePlacements() {
    ...
    for (let r = 0; r < H; r++) {
      for (let c = 0; c < W; c++) {
        ...
        cellToCol.set(k, validCells.length);
        validCells.push(k);
      }
    }
    const pieceCols = {}; let colIndex = validCells.length;
    for (const p of PIECE_ORDER) {
      if (enabledPieces.has(p)) pieceCols[p] = colIndex++;
    }
  ...
  function exactCoverSolve(limit = 1, preselectRows = null) {
    ...
    for (let r = 0; r < rows.length; r++) {
      ...
      for (let j = 0; j < colsInRow.length; j++) {
        ...
        if (first) { linkLR(first.L, node); linkLR(node, first); }
        else { first = node; first.L = first.R = first; }
      }
      rowNodes.push(first);
    }
    function cover(col) {
    ...
    function chooseColumn() {
      ...
      for (let c = header.R; c !== header; c = c.R) {
        if (c.S < s) { s = c.S; best = c; if (s <= 1) break; }
      }
      return best;
    }
    ...
    function search() {
      if (header.R === header) {
        solutions.push(solution.slice().map(r => placements[r]));
        return;
      }
      const col = chooseColumn();
      if (!col || col.S === 0) return;
      cover(col);
  ...
  shuffleBtn.addEventListener('click', () => {
    ...
    setStatus('Ordine di ricerca mescolato. Usa “Risolvi (1)” o “Trova tutte”.');
  });
  ...
  suggestBtn.addEventListener('click', () => {
    ...
    const valid = W * H - holes.size;
    if (need !== valid) {
      setStatus(`⛔ Area incoerente: celle valide=${valid}, richieste=${need}.`);
      return;
    }
    const pre = preselectRowsForPlaced();
    if (pre === null) {
      setStatus('⛔ Posizionamenti manuali incoerenti.');
      return;
    }
    const sols = exactCoverSolve(1, pre);
    if (!sols.length) {
      setStatus('Nessuna soluzione compatibile.');
      return;
    }
    const placedSet = new Set(placed.keys());
    const next = sols[0].find(pl => !placedSet.has(pl.piece));
    if (!next) {
      setStatus('Tutti i pezzi già posizionati.');
      return;
    }
    ...
    renderBoard();
    setStatus(`Suggerimento: posato ${next.piece}.`);
  });
  ...
  solveOneBtn.addEventListener('click', () => {
    ...
    placed.clear();
    for (const pl of sols[0]) {
      placed.set(pl.piece, { cells: pl.cells.slice(), shape: null, r0: 0, c0: 0 });
    }
    renderBoard();
    ...
    for (let t = 0; t < n; t++) {
      ...
      for (let r = 0; r < H; r++) {
        for (let c = 0; c < W; c++) {
          ...
          const k = r * W + c;
          for (const pl of sol) {
            if (pl.cells.includes(k)) {
              div.classList.add('p');
              div.style.background = thumbColor(pl.piece);
              break;
            }
          }
          if (holes.has(k)) div.style.background = '#1b0f0f';
          mini.appendChild(div);
        }
      }
      const cap = document.createElement('div'); cap.className = 'cap'; cap.textContent = `#${t + 1}`;
      wrap.appendChild(mini); wrap.appendChild(cap);
      el.appendChild(wrap);
    }
  }
  ...
  prevBtn.addEventListener('click', () => {
    ...
    previewSolution(solIdx);
    setStatus(`Anteprima ${solIdx + 1}/${foundSolutions.length}.`);
  });
  ...
  applyBtn.addEventListener('click', () => {
    if (!foundSolutions.length) return;
    placed.clear();
    for (const pl of foundSolutions[solIdx]) {
      placed.set(pl.piece, { cells: pl.cells.slice(), shape: null, r0: 0, c0: 0 });
    }
    renderBoard();
